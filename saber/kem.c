...
int PQCLEAN_NAMESPACE_crypto_kem_keypair(unsigned char *pk, unsigned char *sk) {
    ...
    PQCLEAN_NAMESPACE_indcpa_kem_keypair(pk, sk);                         // sk[0:SABER_INDCPA_SECRETKEYBYTES-1] <-- sk
...
int PQCLEAN_NAMESPACE_crypto_kem_enc(unsigned char *ct, unsigned char *ss, const unsigned char *pk) {
    ...
    PQCLEAN_NAMESPACE_indcpa_kem_enc(buf, kr + 32, pk,  ct); // buf[0:31] contains message; kr[32:63] contains randomness r;
...
int PQCLEAN_NAMESPACE_crypto_kem_dec(unsigned char *ss, const unsigned char *ct, const unsigned char *sk) {
    ...
    PQCLEAN_NAMESPACE_indcpa_kem_dec(sk, ct, buf);               // buf[0:31] <-- message
    ...
    PQCLEAN_NAMESPACE_indcpa_kem_enc(buf, kr + 32, pk, cmp);


    fail = PQCLEAN_NAMESPACE_verify(ct, cmp, SABER_BYTES_CCA_DEC);

    sha3_256(kr + 32, ct, SABER_BYTES_CCA_DEC);                 // overwrite coins in kr with h(c)

    PQCLEAN_NAMESPACE_cmov(kr, sk + SABER_SECRETKEYBYTES - SABER_KEYBYTES, SABER_KEYBYTES, fail);
