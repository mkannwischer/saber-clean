...
static void GenMatrix(polyvec *a, const unsigned char *seed) {
    ...
    for (i = 0; i < SABER_K; i++) {
        for (j = 0; j < SABER_K; j++) {
            PQCLEAN_NAMESPACE_BS2POL(buf + (i * SABER_K + j) * (13 * SABER_N / 8), temp_ar);
            for (k = 0; k < SABER_N; k++) {
...
void PQCLEAN_NAMESPACE_indcpa_kem_keypair(unsigned char *pk, unsigned char *sk) {
    ...
    PQCLEAN_NAMESPACE_GenSecret(skpv, noiseseed); //generate secret from constant-time binomial distribution
    ...
    PQCLEAN_NAMESPACE_POLVEC2BS(sk, skpv, SABER_Q);

    //------------------unload and pack pk=256 bits seed and 3 x (256 coefficients of 11 bits)-------


    PQCLEAN_NAMESPACE_POLVEC2BS(pk, res, SABER_P); // load the public-key coefficients
...
void PQCLEAN_NAMESPACE_indcpa_kem_enc(const unsigned char *message_received, unsigned char *noiseseed, const unsigned char *pk, unsigned char *ciphertext) {
    uint32_t i, j, k;
    polyvec a[SABER_K];     // skpv;
    unsigned char seed[SABER_SEEDBYTES];
    ...
    PQCLEAN_NAMESPACE_GenSecret(skpv1, noiseseed); //generate secret from constant-time binomial distribution
    ...
    PQCLEAN_NAMESPACE_POLVEC2BS(ciphertext, res, SABER_P);
...
//*******************client matrix-vector multiplication ends************************************
    ...
    PQCLEAN_NAMESPACE_BS2POLVEC(pk, pkcl, SABER_P);
    ...
    #if Saber_type == 1
    PQCLEAN_NAMESPACE_pack_3bit(msk_c, vprime);
    #elif Saber_type == 2
    PQCLEAN_NAMESPACE_pack_4bit(msk_c, vprime);
    #elif Saber_type == 3
    PQCLEAN_NAMESPACE_pack_6bit(msk_c, vprime);
    #endif
...
void PQCLEAN_NAMESPACE_indcpa_kem_dec(const unsigned char *sk, const unsigned char *ciphertext, unsigned char message_dec[]) {
    ...
    PQCLEAN_NAMESPACE_BS2POLVEC(sk, sksv, SABER_Q); //sksv is the secret-key
    PQCLEAN_NAMESPACE_BS2POLVEC(ciphertext, pksv, SABER_P); //pksv is the ciphertext
    ...
    #if Saber_type == 1
    PQCLEAN_NAMESPACE_un_pack3bit(scale_ar, op);
    #elif Saber_type == 2
    PQCLEAN_NAMESPACE_un_pack4bit(scale_ar, op);
    #elif Saber_type == 3
    PQCLEAN_NAMESPACE_un_pack6bit(scale_ar, op);
    #endif
...
static void MatrixVectorMul(polyvec *a, uint16_t skpv[SABER_K][SABER_N], uint16_t res[SABER_K][SABER_N], uint16_t mod, int16_t transpose) {
    ...
    if (transpose == 1) {
        for (i = 0; i < SABER_K; i++) {
            for (j = 0; j < SABER_K; j++) {
                PQCLEAN_NAMESPACE_pol_mul((uint16_t *)&a[j].vec[i], skpv[j], acc, SABER_Q, SABER_N);
        ...
        for (i = 0; i < SABER_K; i++) {
            for (j = 0; j < SABER_K; j++) {
                PQCLEAN_NAMESPACE_pol_mul((uint16_t *)&a[i].vec[j], skpv[j], acc, SABER_Q, SABER_N);
                for (k = 0; k < SABER_N; k++) {
                    res[i][k] = res[i][k] + acc[k];
...
static void InnerProd(uint16_t pkcl[SABER_K][SABER_N], uint16_t skpv[SABER_K][SABER_N], uint16_t mod, uint16_t res[SABER_N]) {
    ...
    // vector-vector scalar multiplication with mod p
    for (j = 0; j < SABER_K; j++) {
        PQCLEAN_NAMESPACE_pol_mul(pkcl[j], skpv[j], acc, SABER_P, SABER_N);
